...
const App = () => {
  ...
  useEffect(() => {
    const checkThankYouPage = () => {
      ...
      const thankYouParam = searchParams.get('thank-you');
      const dataParam = searchParams.get('data');
      
      // Check if URL contains thank-you parameter and data
      if (thankYouParam === '1' && dataParam) {
    ...
    // Listen for popstate (browser back/forward) and hash changes
  ...
  const handleProductSubmit = useCallback((newProduct) => {
    ...
    const productDisplayName = newProduct.model_name || newProduct.productName || 'product';
    showAlertMessage(
      `Successfully generated ${newProduct.units.length} QR codes for ${productDisplayName}!`
    );
  ...
  // If thank you page should be shown, render it instead
  if (showThankYou) {
    return <ThankYou />;
  }
